  ...
  <div class="register-job main-job-container">
    <el-form ref="jobForm" :model="jobForm" :rules="jobRules" autocomplete="off" label-position="left">
      <div class="breadcrumb-box d-flex job-title-pc">
        ...
        <img class="breadcrumb-arrow" src="../../../assets/icons/breadcrumb-arrow.svg" />
        <span class="breadcrumb-current">{{ $t('job_list.edit_job') }}</span>
      </div>
      <div @click="$router.push('/list-job')" class="job-title-sp d-flex items-center cursor-pointer">
        <img src="../../../assets/icons/prev_mobile.svg" />
        <span class="breadcrumb-current">{{ $t('my_page.job_edit') }}</span>
      </div>
      <div v-show="!jobDelete" class="register-job__body">
        ...
        <div class="register-job__table">
          <div class="register-job__col">
            <div class="register-job__left d-flex justify-between items-center">
              <span>{{ $t('job_list.status') }}</span>
              <div class="required-field">{{ $t('form.required') }}</div>
            </div>
            <div ref="store_id" class="register-job__right select-job-type items-center">
              <el-form-item label="" prop="job_status_id" :error="(error.key === 'job_status_id') ? error.value : ''">
                <el-select
                  class="w-320 resize-width"
                  v-model="jobForm.job_status_id"
                  :placeholder="$t('job_list.enter_place')"
                  @change="validate('job_status_id')"
                  @focus="resetValidate('job_status_id')"
                  @visible-change="(event) => { checkValidate('job_status_id', event) }"
                >
                  <div v-if="listStatus.length === 0">
                    <el-option :label="$t('common.select')" :value="$t('job_list.enter_place')" :disabled="true"></el-option>
                  </div>
                  <el-option v-for="item in listStatus" :key="item.id" :label="item.name"
                             :value="item.id">
                  </el-option>
                </el-select>
              </el-form-item>
          ...
          <div class="register-job__col">
            ...
            <div ref="pick_up_point" class="register-job__right d-flex flex-col">
              <el-form-item label="" prop="pick_up_point" :error="(error.key === 'pick_up_point') ? error.value : ''">
                <el-input class="" type="textarea" :rows="3" v-model="jobForm.pick_up_point" maxlength="2000"
                          :placeholder="$t('my_page.pick_up_point')" :disabled="disabledInput" @focus="resetValidate('pick_up_point')"/>
              </el-form-item>
              ...
              <el-form-item label="" prop="job_banner" :error="(error.key === 'job_banner') ? error.value : ''">
                <div class="show-avatar">
                  <div class="button-upload">
                    <el-button class="upload-button" :disabled="disabledInput">
                      <label v-if="disabledInput">{{ $t('my_page.select_file') }}</label>
                      <label v-else for="upload-avatar">{{ $t('my_page.select_file') }}</label>
                    </el-button>
                  ...
                  <div v-if="imageAvatarShow" class="show-detail">
                    <div v-if="imageAvatarShow" id="img-avatar" class="show-image">
                      <el-image :src="imageAvatarShow" :preview-src-list="[imageAvatarShow]" />
                    </div>
                    <img v-show="!disabledInput" class="image-close" src="/assets/icon/icon_close_image.svg" alt="" @click="removeAvatar">
                  </div>
          ...
          <div class="register-job__col">
            ...
            <div ref="job_thumbnails" class="register-right flex-col">
              <el-form-item label="" prop="job_thumbnails" :error="(error.key === 'job_thumbnails') ? error.value : ''">
                ...
                <div class="button-upload">
                  <el-button class="upload-button" :disabled="disableImgUp || disabledInput">
                    <label v-if="disableImgUp || disabledInput" class="cursor-not-allowed">{{ $t('my_page.select_file') }}</label>
                ...
                <div v-if="imageDetailShow.length" class="d-flex show-img-detail">
                  <div v-for="(detail, index) in imageDetailShow" :key="index" class="show-detail">
                    <div v-if="detail" class="sub-image">
                      <el-image :src="detail" :preview-src-list="[detail]" />
                    </div>
                    <img v-show="!disabledInput" class="image-close" src="/assets/icon/icon_close_image.svg" alt="" @click="removeImage(index)">
                  </div>
                </div>
              </el-form-item>
            </div>
        ...
        <div class="register-job__table">
          ...
          <div class="register-job__col">
            ...
            <div ref="description" class="register-job__right d-flex flex-col">
              <el-form-item label="" prop="description"
                ...
                <el-input class="" tabindex="2" maxlength="2000" @focus="resetValidate('description')"
            ...
            <div ref="work_type_ids" class="register-job__right d-flex items-center">
              <div class="register-job__checkbox">
                <el-form-item label="" prop="work_type_ids" :error="(error.key === 'work_type_ids') ? error.value : ''">
                  <el-checkbox-group v-model="jobForm.work_type_ids" :disabled="disabledInput" @change="validate('work_type_ids')">
                ...
                <el-form-item :error="errors.salary_min_max">
                  <div class="form-min-max">
                    <el-form-item class="w-120 resize-width" label="" prop="salary_min" :error="(error.key === 'salary_min') ? error.value : ''">
                      <el-input v-model="jobForm.salary_min" class="" :disabled="disabledInput" :placeholder="$t('job_list.enter_place')" @focus="resetValidate('salary_min')" @change="checkMinMax"/>
                    </el-form-item>
                    ...
                    <el-form-item class="w-120 resize-width" label="" prop="salary_max" :error="(error.key === 'salary_max') ? error.value : ''">
                      <el-input v-model="jobForm.salary_max" :disabled="disabledInput" :placeholder="$t('job_list.enter_place')" @focus="resetValidate('salary_max')" @change="checkMinMax"/>
                    </el-form-item>
                  </div>
                </el-form-item>
              </div>
          ...
          <div class="register-job__col">
            ...
            <div ref="salary_description" class="register-job__right d-flex flex-col">
              <el-form-item label="" prop="salary_description"
                ...
                <el-input class="" type="textarea" maxlength="2000" :rows="3" v-model="jobForm.salary_description" :disabled="disabledInput" @focus="resetValidate('salary_description')"
            ...
            <div class="register-job__right d-flex items-center">
              <div class="register-job__checkbox">
                <el-form-item label="" prop="working_days" :error="(error.key === 'working_days') ? error.value : ''">
                  <el-checkbox-group v-model="jobForm.working_days" :disabled="disabledInput">
          ...
          <div class="register-job__col">
            ...
            <div class="register-job__right d-flex flex-col">
              <el-form-item class="resize-width" label="" prop="shifts" :error="(error.key === 'shifts') ? error.value : ''">
                <el-input class="" type="textarea" :rows="3" v-model="jobForm.shifts" maxlength="2000" :disabled="disabledInput"
            ...
            <div ref="gender_ids" class="register-job__right gender-checkbox">
              <div class="register-job__gender">
                <el-form-item label="" prop="gender_ids" :error="(error.key === 'gender_ids') ? error.value : ''">
                  <el-checkbox-group v-model="jobForm.gender_ids" :disabled="disabledInput">
                ...
                <div class="d-flex g-16 pt-14 form-address-input">
                  ...
                  <div class="d-flex flex-col form-address-data" ref="province_city_id">
                    ...
                    <el-form-item label="" prop="province_city_id" :error="(error.key === 'province_city_id') ? error.value : ''">
                      <el-select
                        ...
                        :disabled="disabledInput || disabledProvinceCity"
                  ...
                  <div ref="address" class="d-flex flex-col form-address-data">
                    ...
                    <el-form-item label="" prop="address" :error="(error.key === 'address') ? error.value : ''">
                      <el-input class="w-130 resize-width" v-model="jobForm.address" maxlength="255" :disabled="disabledInput" :placeholder="$t('common.placeholder.address')" @focus="resetValidate('address')" />
                    </el-form-item>
                  </div>
                  <div class="d-flex flex-col form-address-data">
                    ...
                    <el-form-item label="" prop="building" :error="(error.key === 'building') ? error.value : ''">
                      <el-input class="w-195 resize-width" v-model="jobForm.building" maxlength="255" :disabled="disabledInput" :placeholder="$t('common.placeholder.building')" @focus="resetValidate('building')" />
                    </el-form-item>
                  </div>
                </div>
              </div>
            ...
            <div ref="station1" class="register-job__right">
              <div class="station-select d-flex g-16">
                <el-form-item label="" prop="station_ids" :error="errors.station1">
                  <el-select class="w-160 resize-width" v-model="station1.province" clearable :disabled="disabledInput" :placeholder="$t('job.station_placeholder.province')" @change="(value) => { changeStationProvince(1, value) }" @focus="resetValidateStation('station1')">
                  ...
                  <div class="station-sp">
                    <el-select class="w-160 resize-width" v-model="station1.railway" clearable :disabled="disabledInput || (listStationRailways[1].length === 0)" :placeholder="$t('job.station_placeholder.railway')" @change="(value) => { changeStationRailway(1, value) }" @focus="resetValidateStation('station1')">
                    ...
                    <el-select class="w-160 resize-width" v-model="station1.station" clearable :disabled="disabledInput || (listStations[1].length === 0)" :placeholder="$t('job.station_placeholder.station')" @focus="resetValidateStation('station1')">
            ...
            <div ref="station2" class="register-job__right">
              <div class="station-select d-flex g-16">
                <el-form-item label="" prop="station_ids2" :error="errors.station2">
                  <el-select class="w-160 resize-width" v-model="station2.province" clearable :disabled="disabledInput" :placeholder="$t('job.station_placeholder.province')" @change="(value) => { changeStationProvince(2, value) }" @focus="resetValidateStation('station2')">
                    <div v-if="listStationProvinces.length === 0">
                      <el-option :label="$t('job.station_placeholder.province')" :value="$t('job_list.enter_place')" :disabled="true"></el-option>
                    </div>
                    <el-option v-for="(item, key) in listStationProvinces" :key="key" :label="key" :value="key" :disabled="item.length === 0">
                  ...
                  <div class="station-sp">
                    <el-select class="w-160 resize-width" v-model="station2.railway" clearable :disabled="disabledInput || (listStationRailways[2].length === 0)" :placeholder="$t('job.station_placeholder.railway')" @change="(value) => { changeStationRailway(2, value) }" @focus="resetValidateStation('station2')">
                    ...
                    <el-select class="w-160 resize-width" v-model="station2.station" clearable :disabled="disabledInput || (listStations[2].length === 0)" :placeholder="$t('job.station_placeholder.station')" @focus="resetValidateStation('station2')">
            ...
            <div class="register-job__right">
              <div ref="station3" class="station-select d-flex g-16">
                <el-form-item label="" prop="station_ids3" :error="errors.station3">
                  <el-select class="w-160 resize-width" v-model="station3.province" clearable :disabled="disabledInput" :placeholder="$t('job.station_placeholder.province')" @change="(value) => { changeStationProvince(3, value) }" @focus="resetValidateStation('station3')">
                    <div v-if="listStationProvinces.length === 0">
                      <el-option :label="$t('job.station_placeholder.province')" :value="$t('job_list.enter_place')" :disabled="true"></el-option>
                    </div>
                  ...
                  <div class="station-sp">
                    <el-select class="w-160 resize-width" v-model="station3.railway" clearable :disabled="disabledInput || (listStationRailways[3].length === 0)" :placeholder="$t('job.station_placeholder.railway')" @change="(value) => { changeStationRailway(3, value) }" @focus="resetValidateStation('station3')">
                    ...
                    <el-select class="w-160 resize-width" v-model="station3.station" clearable :disabled="disabledInput || (listStations[3].length === 0)" :placeholder="$t('job.station_placeholder.station')" @focus="resetValidateStation('station3')">
            ...
            <div ref="welfare_treatment_description" class="register-job__right d-flex flex-col">
              <el-form-item label="" prop="welfare_treatment_description" :error="(error.key === 'welfare_treatment_description') ? error.value : ''">
                <el-input class=""  type="textarea" :rows="3" v-model="jobForm.welfare_treatment_description" @focus="resetValidate('welfare_treatment_description')" maxlength="2000"
                          :disabled="disabledInput" :placeholder="$t('job.welfare_treatment')" />
              </el-form-item>
          ...
          <div class="register-job__col">
            ...
            <div class="register-job__right">
              <el-checkbox-group v-model="jobForm.feature_ids" :disabled="disabledInput">
        ...
        <div class="register-job__button">
          <el-button class="send-button" type="danger" @click="updateConfirm">{{ $t('form.update') }}</el-button>
        </div>
      </div>
      <div v-show="jobDelete">
        <div class="detail-delete">{{ $t('no_data.job_detail_delete') }}</div>
      </div>
    </el-form>
...
export default {
    ...
    return {
      ...
      jobRules: {
        name: [
          ...
          { required: true, message: this.$t('validation.required', { _field_: this.$t('my_page.job_title') }), trigger: 'blur' },
        ...
        work_type_ids: [
          { validator: validRequiredArray, message: this.$t('validation.required_select', { _field_: this.$t('job.employment_status') }), trigger: 'blur' }
        ],
        description: [
        ...
        province_city_id: [
          {
            required: true,
            message: this.$t('validation.required', { _field_: this.$t('my_page.municipalities') }),
            trigger: 'blur'
          }
        ],
        address: [
  ...
  computed: {
    ...
    disabledInput() {
      if (this.clonedJobForm.job_status_id) {
        return [this.STATUS_REGISTER, this.STATUS_PRIVATE, this.STATUS_END].includes(this.clonedJobForm.job_status_id)
      }
      return true
    },
    disableImgUp() {
  ...
  methods: {
    ...
    async getMasterData() {
      ...
      if (dataMasterData.status_code === 200) {
        ...
        this.listHourTypes = dataMasterData.data.range_hours_type !== undefined ? dataMasterData.data.range_hours_type : []
        this.listMorning = dataMasterData.data.morning_afternoon !== undefined ? dataMasterData.data.morning_afternoon : []
      }
    ...
    updateConfirm() {
      if ((this.jobForm.job_status_id === this.STATUS_DRAFT)) {
        if ((this.clonedJobForm.job_status_id !== this.jobForm.job_status_id)) {
          this.dialogConfirm = true
          this.stateUpdate = this.$t('content.CON_006')
        } else {
          this.update()
        }
        return
      ...
      this.$refs.jobForm.validate((valid, key) => {
        if (valid && check) {
          if (this.clonedJobForm.job_status_id !== this.jobForm.job_status_id) {
            this.dialogConfirm = true
            this.stateUpdate = this.$t('content.CON_008')
            if (this.jobForm.job_status_id === this.STATUS_REGISTER) {
              this.stateUpdate = this.$t('content.CON_007')
            }
          } else {
            this.update()
          }
    ...
    async updateJob() {
      ...
      try {
        ...
        const response = await this.$store.dispatch(JOB_UPDATE_JOB, { id: this.$route.params.slug, form: dto })
        this.zipCodeInput()
    ...
    async getDetailJob() {
      ...
      switch (response.status_code) {
        case 200:
          ...
          this.jobForm.name = response.data.name
          this.jobForm.store_id = response.data.store_id
          this.jobForm.job_status_id = response.data.job_status_id
          this.jobForm.pick_up_point = response.data.pick_up_point
          this.jobForm.description = response.data.description
          this.jobForm.shifts = response.data.shifts
          this.jobForm.feature_ids = response.data.feature_ids || []
          this.jobForm.range_hours_type = response.data.range_hours_type
          this.jobForm.welfare_treatment_description = response.data.welfare_treatment_description
          this.jobForm.job_banner = response.data.banner_image
          this.imageAvatarShow = response.data.banner_image
          // eslint-disable-next-line no-case-declarations
          const dataImage = this.convertDataToArray(response.data.detail_images || [], 'url')
          this.jobForm.job_thumbnails = dataImage
          this.imageDetailShow = dataImage
          this.jobForm.job_type_ids = this.convertDataToArray(response.data.job_types || [], 'id')
          this.jobForm.working_days = this.convertDataToArray(response.data.working_days || [], 'id')
          this.jobForm.experience_ids = this.convertDataToArray(response.data.experience_types || [], 'id')
          this.jobForm.work_type_ids = this.convertDataToArray(response.data.work_types || [], 'id')
          this.jobForm.gender_ids = this.convertDataToArray(response.data.genders || [], 'id')
          this.jobForm.salary_type_id = response.data.salary.type_id
          this.jobForm.salary_min = response.data.salary.min
          this.jobForm.salary_max = response.data.salary.max
          this.jobForm.start_work_time = response.data.work_time.start
          this.jobForm.salary_description = response.data.salary.description
          if (response.data.work_time.start) {
            this.startHour = response.data.work_time.start_time.hours
            this.startMinute = response.data.work_time.start_time.minute
            this.jobForm.start_work_time_type = response.data.work_time.start_work_time_type
          }
          if (response.data.work_time.end) {
            this.endHour = response.data.work_time.end_time.hours
            this.endMinute = response.data.work_time.end_time.minute
            this.jobForm.end_work_time_type = response.data.work_time.end_work_time_type
          }
          this.jobForm.end_work_time = response.data.work_time.end
          this.jobForm.age_min = response.data.age.min
          this.jobForm.age_max = response.data.age.max
          this.jobForm.postal_code = response.data.address.postal_code
          this.zipCodeInput()
          this.jobForm.province_id = response.data.address.province_id
          if (this.jobForm.province_id) {
            this.listCities = this.listProvinces[response.data.address.province_id].province_city
          }
          this.jobForm.province_city_id = response.data.address.province_city_id
          this.jobForm.address = response.data.address.address
          this.jobForm.building = response.data.address.building
          response.data.stations.forEach((station, index) => {
            this.jobForm.station_ids.push(station.id)
            this['station' + (index + 1)].province = station.province_name
            this.changeStationProvince(index + 1, station.province_name)
            this.changeStationRailway(index + 1, station.railway_name)
            this['station' + (index + 1)].railway = station.railway_name
            this['station' + (index + 1)].station = station.id
          })
